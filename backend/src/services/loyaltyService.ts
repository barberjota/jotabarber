...
import { TipoRecompensa } from '@prisma/client';

export class LoyaltyService {
  /**
   * Procesa la finalización de una cita para otorgar puntos y actualizar el contador de cortes.
   */
  static async processCompletedAppointment(citaId: string): Promise<void> {
   ...
   */
  static async redeemProductWithPoints(userId: string, productId: string, quantity: number = 1): Promise<void> {
    ...
    if (!product.costoPuntos) throw new Error('Este producto no se puede canjear con puntos');
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.historialFidelidad.create({
        data: {
          ...
          tipoRecompensa: TipoRecompensa.CANJE_PUNTOS,
        },
      });
